...
namespace tree
{
  ...
  // REMOÇÃO DE NODOS
  void BST_remove(node *&root, std::string &dsValor)
  {
    if (!root)
      return;
  ...
  void show_level(node *root, int nrLevel)
  {
    if (!root || nrLevel < 1)
      return;
    ...
    // Caso contrário, desce para o próximo nível
    show_level(root->left,  nrLevel - 1);
    show_level(root->right, nrLevel - 1);
  }
}